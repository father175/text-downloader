...
def run(start,id):
    ...
    while True:
            ...
            # 获取文字内容
            output=text.find("div",attrs={"class":"readcontent"}).text
            output=output.replace("\n","")
            output=output.replace("    ","\n  ")
            output=output.replace("本章未完，点击下一页继续阅读","")
            output=output.replace("天才一秒记住顶点小说网,www.xkjxw.com,如果被浏览器转码或畅读,内容容易缺失,阅读体验极差,请退出转码或畅读模式。","")
...
# 任务分配系统
for i in mad:
    print(i["href"]+" be pushed in queue.")
    run(i["href"],i.text)
